...
export default class EditLostDog extends Component {
    ...
    componentDidMount() {
        axios.get('http://localhost:5000/api/lost/'+this.props.match.params.id)
        .then(response => {
          this.setState({
    ...
    deleteLostDog(id) {
        axios.delete('http://localhost:5000/api/lost/'+this.props.match.params.id)
...
axios.post('http://localhost:5000/api/lost/update/'+this.props.match.params.id, dog)
...
// window.location= '/'
    ...
    render() {
        return (
            ...
            <a href="#" onClick={() => { this.deleteLostDog(this.state._id) }}>Удалить</a>
          </div>
          )
        }
      }